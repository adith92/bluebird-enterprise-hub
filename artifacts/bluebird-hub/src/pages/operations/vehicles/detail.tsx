...
import { ChevronLeft, Info, ReceiptText, AlertTriangle } from "lucide-react";
...
const statusFormSchema = z.object({
  ...
  (data) => {
    if (data.status === "maintenance" && !data.maintenanceCompletionDate) {
      return false;
    }
    return true;
  },
...
export default function VehicleDetail() {
  ...
  // Update form default values when vehicle data loads
  useState(() => {
    if (vehicle) {
      form.reset({
        status: vehicle.status as any,
        maintenanceCompletionDate: vehicle.maintenanceCompletionDate ? new Date(vehicle.maintenanceCompletionDate) : undefined,
    ...
    try {
      await updateStatus.mutateAsync({ 
        ...
        data: { 
          ...
          maintenanceCompletionDate: values.status === "maintenance" && values.maintenanceCompletionDate ? values.maintenanceCompletionDate.toISOString() : undefined,
          maintenanceNote: values.status === "maintenance" ? values.maintenanceNote : undefined,
        } 
      });
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold tracking-tight">{vehicle.plateNumber}</h1>
            <Badge 
              variant={
              ...
              className={vehicle.status === "available" ? "bg-green-100 text-green-800" : vehicle.status === "maintenance" ? "bg-amber-100 text-amber-800 border-none" : ""}
            >
              {vehicle.status}
            </Badge>
          </div>
          <p className="text-sm text-muted-foreground">{vehicle.category} • {vehicle.model} ({vehicle.year})</p>
        </div>
        <div>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmitStatus)} className="space-y-4 pt-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-col">
                            <FormLabel>Expected Completion Date <span className="text-destructive">*</span></FormLabel>
                            <Popover>
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t">
                    <Button variant="outline" type="button" onClick={() => setStatusDialogOpen(false)}>Cancel</Button>
                    <Button type="submit" disabled={updateStatus.isPending}>Save Changes</Button>
                  </div>
                </form>
              </Form>
            </DialogContent>
      ...
      {vehicle.status === "maintenance" && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3 items-start text-amber-800">
          ...
          <div>
            ...
            <p className="text-sm mt-1">Expected completion: <strong>{formatDate(vehicle.maintenanceCompletionDate)}</strong></p>
            {vehicle.maintenanceNote && (
              <p className="text-sm mt-2 p-2 bg-amber-100/50 rounded rounded-md italic">"{vehicle.maintenanceNote}"</p>
            )}
          </div>
        </div>
      )}
